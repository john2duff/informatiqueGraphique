#include "../base.c"
#define ATTENTE 300000 //A ajuster pour la vitesse d'animation
char name[]="Primitives graphique OpenGL";
char message[200];
int scene=0;
int parcours=1; //demarrage avec l'animation des primitives
int attente=0;  //temps d'attente en cours
...
void description(){
    ...
    glColor3dv(VIOLET);glVertex3dv(p6);glColor3dv(TURQUOISE);glVertex3dv(p7);
    glEnd();
    //Affiche les points
    ...
    affiche_caractere(p0,'0',NOIR);affiche_caractere(p1,'1',NOIR);affiche_caractere(p2,'2',NOIR);
    affiche_caractere(p3,'3',NOIR);affiche_caractere(p4,'4',NOIR);affiche_caractere(p5,'5',NOIR);
...
void initialisation(){
    ...
    glutAddMenuEntry("GL_POINTS",0);glutAddMenuEntry("GL_LINES",1);
    glutAddMenuEntry("GL_LINE_STRIP",2);glutAddMenuEntry("GL_LINE_LOOP",3);
    glutAddMenuEntry("GL_QUADS",4);glutAddMenuEntry("GL_QUAD_STRIP",5);
    glutAddMenuEntry("GL_POLYGON",6);glutAddMenuEntry("GL_TRIANGLES",7);
    glutAddMenuEntry("GL_TRIANGLE_STRIP",8);glutAddMenuEntry("GL_TRIANGLE_FAN",9);
    glutAddMenuEntry("ANIMATION",10);glutAttachMenu(GLUT_RIGHT_BUTTON);
    information=1;
}
