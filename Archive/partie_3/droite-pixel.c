...
void traceDroite_v1(int x0,int y0,int dx,int dy,double *c){
	...
	if ((dx==0)&&(dy==0)){
        glBegin(GL_POINTS);
        glVertex2d(xint2coord(x0),yint2coord(y0));
        glEnd();
		return;
	}
    glBegin(GL_POINTS);
	if (abs(dx) > abs(dy)){
		...
		for (i=0;i<=dx;i++){
            glVertex2d(xint2coord(x0+i),yint2coord(y0+m*i));
		...
		for (i=0;i<=dy;i++){
            glVertex2d(xint2coord(x0+m*i),yint2coord(y0+i));
		}
	}
    glEnd();
}
...
void traceDroite_v2(int x0,int y0,int dx,int dy,double *c){
	...
	if (abs(dx) > abs(dy)){
        ...
        y = (double) y0;
		for (i=0;i<=dx;i++){
            glVertex2d(xint2coord(x0+i),yint2coord(y));
			y=y + m;
		}
    }
	else{
        ...
        x = (double) x0;
		for (i=0;i<=dy;i++){
            glVertex2d(xint2coord(x),yint2coord(y0+i));
			x = x + m;
		}
	}
    glEnd();
    ...
    if (abs(dx)>abs(dy)){
        ...
        for(i=1;i<=dx;i++){
            if (d<=0) {
                ...
                d=d+incNA;
                y0=y0+inc; }
            glVertex2d(xint2coord(x0+i),yint2coord(y0));
		}
	}
    else {
        if (dy<0){
        ...
        for(i=1;i<=dy;i++){
            if (d<=0){
                ...
                d=d+incNA;
                x0=x0+inc;}
            glVertex2d(xint2coord(x0),yint2coord(y0+i));
        }
    }
    glEnd();
}
...
void traceDroite_v4(int x0,int y0,int dx,int dy,double *c){
    ...
    if (abs(dx)>abs(dy)){
        ...
        glColor3dv(cf);
        glVertex2d(xint2coord(x0),yint2coord(y0+1));
        glColor3dv(cf);
        glVertex2d(xint2coord(x0),yint2coord(y0-1));
        for(i=1;i<=dx;i++){
            if (d<=0) {
            ...
            glColor3dv(cf);
            glVertex2d(xint2coord(x0+i),yint2coord(y0));
            filtre((double) (2 *dx + D) / (2.0 * sqrt((double)(dx*dx)+(double)
                                                      (dy*dy))),c,cf);
            glColor3dv(cf);
            glVertex2d(xint2coord(x0+i),yint2coord(y0-1));
            filtre((double) (2 *dx - D) / (2.0 * sqrt((double)(dx*dx)+
                                                      (double)(dy*dy))),c,cf);
            glColor3dv(cf);
            glVertex2d(xint2coord(x0+i),yint2coord(y0+1));
        }
