#include "../base.c"
char name[]="Remplissage polygones convexes";
char message[200];
...
void remplirPolygone(arete *p,int taille,GLdouble *couleur){
	...
	for (i=0;i<DIVISION;i++){
        ta[i][DROITE][POS_INUTILISE]=???;
        ta[i][GAUCHEDROITE][POS_INUTILISE]=???;
	}
	...
	for (i=0;i<taille;i++){
		...
		if (p[i][YBAS]<ymin) ymin = ???;
		if (p[i][YHAUT]>ymax) ymax = ???;
	}
	...
	mg=	(double)(taa[GAUCHE][XHAUT]-taa[GAUCHE][XBAS])/ (double)(taa[GAUCHE][YHAUT]-taa[GAUCHE][YBAS]);
	md=	(double)(taa[DROITE][XHAUT]-taa[DROITE][XBAS])/(double)(taa[DROITE][YHAUT]-taa[DROITE][YBAS]);
	for(y=ymin;y<ymax;y++){
