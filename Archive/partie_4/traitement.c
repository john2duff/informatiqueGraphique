...
GLubyte ET (GLubyte a,GLubyte b){ return (a & b);}
GLubyte OU (GLubyte a,GLubyte b){ return (a | b);}
GLubyte XOU(GLubyte a,GLubyte b){ return (a ^ b);}
GLubyte NON(GLubyte a,GLubyte b){	return (~a);}
GLubyte OUI(GLubyte a,GLubyte b){	return (a);}
GLubyte INVERSE(GLubyte a,GLubyte b){	return (255-a);}
GLubyte LUMM(GLubyte a,GLubyte b){	
	double r = (double)(a)/255;
	r = pow(r,1.4);
	return (GLubyte) (r * 255.0);
}
...
GLubyte CONTM(GLubyte a,GLubyte b){
	...
	if (r<0.5){
		r = pow(2*r,0.7);
		return (GLubyte) (r * 127.0);
	}		
	else{
		r = pow((r-0.5)*2,1.4);
		return (GLubyte) (127.0 + (r * 127.0));
	}
}
...
void monochrome(typeImage src,typeImage dest){
  ...
  for ( y = 0 ; y < TAILLE ; y++ ) //largeur
  	for ( x = 0 ; x < TAILLE ; x++ ){ //hauteur
			DEST(x,y,0)=	(SRC(x , y ,0)+SRC(x , y ,1)+SRC(x , y ,2))/3 ;
			DEST(x,y,1)=DEST(x,y,2)=DEST(x,y,0);
		}
	#undef SRC
	#undef DEST
}
...
void operationBase(GLubyte (*operateur)(GLubyte,GLubyte),typeImage src1,typeImage src2,typeImage dest){
  ...
  for ( y = 0 ; y < TAILLE ; y++ ) //largeur 
  	for ( x = 0 ; x < TAILLE ; x++ ){ //hauteur
  		DEST(x,y,0)=	operateur(SRC1(x , y ,0),SRC2(x , y ,0));
  		DEST(x,y,1)=	operateur(SRC1(x , y ,1),SRC2(x , y ,1));
  		DEST(x,y,2)=	operateur(SRC1(x , y ,2),SRC2(x , y ,2));
		}
	#undef SRC
	#undef DEST
}
...
void filtreMatriciel(typeFiltre f,typeImage src,typeImage dest){
  ...
  for ( y = 1 ; y < TAILLE-1 ; y++ ) //largeur 
  	for ( x = 1 ; x < TAILLE-1 ; x++ ){ //hauteur
			DEST(x,y,0)=	
				f[0][0]*SRC(x-1,y-1,0)+f[1][0]*SRC(x-1,y,0)+f[2][0]*SRC(x-1,y+1,0)+
				f[0][2]*SRC(x+1,y-1,0)+f[1][2]*SRC(x+1,y,0)+f[2][2]*SRC(x+1,y+1,0)+
				f[0][1]*SRC(x  ,y-1,0)+f[1][1]*SRC(x  ,y,0)+f[2][1]*SRC(x  ,y+1,0);
			DEST(x,y,1)=	
				f[0][0]*SRC(x-1,y-1,1)+f[1][0]*SRC(x-1,y,1)+f[2][0]*SRC(x-1,y+1,1)+
				f[0][2]*SRC(x+1,y-1,1)+f[1][2]*SRC(x+1,y,1)+f[2][2]*SRC(x+1,y+1,1)+
				f[0][1]*SRC(x  ,y-1,1)+f[1][1]*SRC(x  ,y,1)+f[2][1]*SRC(x  ,y+1,1);
			DEST(x,y,2)=	
				f[0][0]*SRC(x-1,y-1,2)+f[1][0]*SRC(x-1,y,2)+f[2][0]*SRC(x-1,y+1,2)+
				f[0][2]*SRC(x+1,y-1,2)+f[1][2]*SRC(x+1,y,2)+f[2][2]*SRC(x+1,y+1,2)+
				f[0][1]*SRC(x  ,y-1,2)+f[1][1]*SRC(x  ,y,2)+f[2][1]*SRC(x  ,y+1,2);
		}
	#undef SRC
	#undef DEST
}
