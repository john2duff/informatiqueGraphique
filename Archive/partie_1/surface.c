...
void surfaceBicubique(int n,matrice M, matrice MP,matrice X,matrice Y,matrice Z,
    ...
    for ( i = 0 ; i < n ; i++ ) {
        ...
        vecteur DX,DY,DZ;
        produitVecteurMatrice(N,S,TX,DX) ; // DX <- S . tx = S m .a . (mp)T
        produitVecteurMatrice(N,S,TY,DY) ;
        produitVecteurMatrice(N,S,TZ,DZ) ;
        for ( j = 0 ; j < n ; j++ ) {
            double t =(double) j/(double) (n-1) ;
            vecteur T = { t*t*t,t*t,t,1.0F } ;
            double x = produitVecteurVecteur(N,DX,T) ;
            double y = produitVecteurVecteur(N,DY,T) ;
            double z = produitVecteurVecteur(N,DZ,T) ;
            glVertex3d(x,y,z);
		}
	}
	glEnd();
