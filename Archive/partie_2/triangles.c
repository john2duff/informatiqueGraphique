...
//Remplir tableau pour un rectangle forme de trois points
...
void afficheTab(){
    ...
    for (i=0;i<taille_fenetre_x;i++)
        for (j=0;j<taille_fenetre_y;j++){
            glColor3d(COULR(i,j),COULV(i,j),COULB(i,j));
            glVertex3d(xint2coord(i),yint2coord(j),PROF(i,j));
        }
    glEnd();
}
...
void remplirRec(double *p1,double *p2,double *p3,double *coul){
    ...
    for (x=p1[0];x<p2[0];x=x+2.0/taille_fenetre_x){
        ...
        for (y=y1;y<y2;y=y+2.0/taille_fenetre_y){
            ...
            if (PROF(xcoord2int(x),ycoord2int(y))< z){
                PROF(xcoord2int(x),ycoord2int(y))= z;
                COULR(xcoord2int(x),ycoord2int(y))= coul[0];
                COULV(xcoord2int(x),ycoord2int(y))= coul[1];
                COULB(xcoord2int(x),ycoord2int(y))= coul[2];
            }
        }
    }
