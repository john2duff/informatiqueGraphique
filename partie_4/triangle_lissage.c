...
char name[]="Triangle lisse Gouraud/Phong";
char message[200];
...
Point L={2,2,2}; //Position lumiere
Point O={0,0,2};//Position observateur
...
double interpolationReel(double y, double ymin, double ymax, double ipb,double iph){
	...
	return ipb + (y-ymin)*(iph-ipb)/(ymax-ymin);
}
...
void interpolationVecteur(double y, double ymin, double ymax,
	...
	res[0]= vb[0]*(1-coeff) + coeff*vh[0];
	res[1]= vb[1]*(1-coeff) + coeff*vh[1];
	res[2]= vb[2]*(1-coeff) + coeff*vh[2];
	Normaliser(res,res);
}

double LumiereDiffuse(Vecteur N,Vecteur L){
	double prodscal= ProduitScalaire(N,L);
	return (MAX(0,prodscal));
}
double LumiereSpeculaire(Vecteur N,Vecteur L, Vecteur O){
	double deuxnl;
	Vecteur R;
	deuxnl = 2 * ProduitScalaire(N,L);
	MultiplicationScalaire(deuxnl,N,R);
	SoustractionVecteur(R,L,R);
	Normaliser(R,R);
	double prodscal= ProduitScalaire(R,O);
	prodscal =MAX(0,prodscal);
	prodscal=pow(prodscal,10);
	return (prodscal);
}
...
void Gouraud(Vecteur N, Point L, Point O){
	...
	Vecteur L1,L2,L3,O1,O2,O3;
	PointVecteurNormaliser(p1,L,L1);
	PointVecteurNormaliser(p2,L,L2);
	PointVecteurNormaliser(p3,L,L3);
	PointVecteurNormaliser(p1,O,O1);
	PointVecteurNormaliser(p2,O,O2);
	PointVecteurNormaliser(p3,O,O3);
    
	double p1id=LumiereDiffuse(N,L1);
	double p1is=LumiereSpeculaire(N,L1,O1);
	double p1i = MIN(1,p1is+p1id);
	double p2id=LumiereDiffuse(N,L2);
	double p2is=LumiereSpeculaire(N,L2,O2);
	double p2i = MIN(1,p2is+p2id);
	double p3id=LumiereDiffuse(N,L3);
	double p3is=LumiereSpeculaire(N,L3,O3);
	double p3i = MIN(1,p3is+p1id);
	//Parcours du triangle
	double i,j;
	for (i=-1;i <= 1; i=i+incl){
		//Calcul intensite en pA et pB diffuse et speculaire
		double pai=interpolationReel(i,-1,1,p1i,p3i);
		double pbi=interpolationReel(i,-1,1,p1i,p2i);
		for (j=-i;j <= 1; j=j+incc){
			//Position de point courant
			pp[0]=j;pp[1]=i;
			//Intensité au point courant
			double ppi=interpolationReel(j,-i,1,pai,pbi);
...
void Gouraud2(Vecteur N1,Vecteur N2,Vecteur N3, Point L, Point O){
	...
	double p1id=LumiereDiffuse(N1,L1);
	double p1is=LumiereSpeculaire(N1,L1,O1);
	double p1i = MIN(1,p1is+p1id);
	double p2id=LumiereDiffuse(N2,L2);
	double p2is=LumiereSpeculaire(N2,L2,O2);
	double p2i = MIN(1,p2is+p2id);
	double p3id=LumiereDiffuse(N3,L3);
	double p3is=LumiereSpeculaire(N3,L3,O3);
	double p3i = MIN(1,p3is+p1id);
	//Parcours du triangle
	double i,j;
	for (i=-1;i <= 1; i=i+incl){
...
void Phong(Vecteur N,Point L, Point O){
	...
	for (i=-1;i <= 1; i=i+incl){
		for (j=-i;j <= 1; j=j+incc){
			...
			PointVecteurNormaliser(pp,L,Lp);
			PointVecteurNormaliser(pp,O,Op);
			double ppid=LumiereDiffuse(N,Lp);
			double ppis=LumiereSpeculaire(N,Lp,Op);
...
void Phong2(Vecteur N1,Vecteur N2,Vecteur N3,Point L, Point O){
	...
	for (i=-1;i <= 1; i=i+incl){
		interpolationVecteur(i, -1, 1,N1,N3,Na);
		interpolationVecteur(i, -1, 1,N1,N2,Nb);
		for (j=-i;j <= 1; j=j+incc){
			pp[0]=j;pp[1]=i;
			PointVecteurNormaliser(pp,L,Lp);
			PointVecteurNormaliser(pp,O,Op);
			//Interpolation des normales
			interpolationVecteur(j, -i, 1,Na,Nb,Np);
			double ppid=LumiereDiffuse(Np,Lp);
			double ppis=LumiereSpeculaire(Np,Lp,Op);
